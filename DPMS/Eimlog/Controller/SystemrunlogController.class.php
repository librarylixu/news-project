...
*/
class SystemrunlogController extends CommonController {
   public $model;//数据库对象
   function __construct(){  
       parent::__construct();  
       $this->model=new \Model\mSystemrunlogModel();
   }
   function  __destruct(){     
      unset($this->model);
   }
    ...
    public function index(){
        $this->assign("mainController","eimSystemrunlogController");
		$this->display('Eimlog@Systemrunlog:index-angular');
    }   
	//插入数据
    //直接传text = '添加设备[EIM-min];结果：成功'
	function MInsert($text=array(),$level=0){
        $parameter['text'] = $text;
        $parameter['time'] = time();
