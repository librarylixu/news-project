window.addEventListener('load', function(){
    ...
    function broadcast(file, playtype) {
        ...
        var gateway = sessioncenterip ? sessioncenterip : window.location.host;
        if (gateway.length < 1) gateway = 'localhost';
        var url = protocol + gateway + ':1336/PLAY?f=' + file;
        if (player){
            player.close();
        }
        ...
        player.run();
        window.document.title = playtype+'会话回放';
    }
