...
appModule.controller('crmWorkordercloseController', ['$scope', '$q', 'dataService', function ($scope, $q, dataService) {
    ...
    $scope.select = function () {
        ...
        params.append('$fieldkey', 'idworkorder,title,createtime,endtime,userid,assignid,refcustomerid,status,guid,feedback,timely,refusers,additional,totalscore');
        //组件项目数据源
    ...
    */
    ...
    //查看评分模态框
    $scope.workerorderselectscoreModal = new Canvi({
        content: ".js-canvi-content",
        isDebug: !1,
        navbar: ".workerordermodalselectscore",
    ...
    //查看评分
    $scope.selectscore = function (data) {
        var workorderdata = angular.copy(data);
        $scope.selectItem = workorderdata;
        $scope.workerorderselectscoreModal.open();
        $scope.workerorderselectscoreModal.title = workorderdata.title.slice(0, 10);
    }
}]);
