...
.controller('eimSystemlogController', ['$scope', 'dataService', '$q', function ($scope, dataService, $q) {
    ...
    $scope.__setPageControl = function () {
        $scope._thisPageArray = [];
        for (var i = -2; i < 3; i++) {
            if (($scope.service._thisPageCount + i) <= Math.ceil($scope.service.systemlogData['count'] / $scope._pageCount)) {
    ...
    */
    $scope.select = function (flag) {
        ...
        if (flag == 1 || $scope.service.systemlogData == undefined) {
            //$scope.service.systemlogData['count'] = $scope.service.systemlogData['count'];
            select_systemlog(params).then(function (res) {
    ...
    */
    $scope.operationlog = function (type) {
        ...
        var url = '';
        if (type == 0) {
            url = __URL + 'Eimlog/Systemlog/select_page_data';
        } else if (type == 1) {
            //如果是清空日志的话需要额外判断
        ...
        $scope.service.postData(url, params).then(function (data) {
            if (type == 0) {
                layer.msg('数据已刷新', { icon: 6 });
                $scope.service.systemlogData = data;
                $scope.__setPageControl();
            } else {
                if (data.n > 0) {
    ...
    $scope.nextPage = function (pageindex) {
        ...
        else if($scope.service._thisPageCount +pageindex> Math.ceil($scope.service.systemlogData['count'] / $scope._pageCount)) {
            $scope.service._thisPageCount = Math.ceil($scope.service.systemlogData['count'] / $scope._pageCount);
        } else {
