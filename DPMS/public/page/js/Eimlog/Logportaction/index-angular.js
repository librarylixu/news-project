...
.controller('eimPduPortSystemLogController', ['$scope', 'dataService', '$q', function ($scope, dataService, $q) {
    ...
    $scope.__setPageControl = function () {
        $scope._thisPageArray = [];
        for (var i = -2; i < 3; i++) {
            if (($scope.service._thisPageCount + i) <= Math.ceil($scope.service.pduportactionlogData['count'] / $scope._pageCount)) {
    ...
    */
    $scope.select = function (flag) {
        ...
        if (flag == 1 || $scope.service.pduportactionlogData == undefined) {
            $scope.service.postData(__URL + 'Eimlog/Logportaction/select_page_data', params).then(function (data) {
                $scope.service.pduportactionlogData = data;
        ...
        if ($scope.service.pduportactionlogData != undefined) {
            $scope.__setPageControl();
        }
    }
    //查询pdu数据
    ...
    //获取操作状态   0:开启，1:关闭，2:重启
    $scope.getsystenstatus = function (status) {
        var statusname = '';
        if(status == 0){
            statusname = '关闭';
        } else if (status == 1) {
    ...
    */
    $scope.operationlog = function (type) {
        ...
        var url = '';
        if (type == 0) {
            url = __URL + 'Eimlog/Logportaction/select_page_data';
        } else if (type == 1) {
            //如果是清空日志的话需要额外判断
            ...
            url = __URL + 'Eimlog/Logportaction/del_page_data';
        }
    ...
    $scope.nextPage = function (pageindex) {
        ...
        else if ($scope.service._thisPageCount + pageindex > Math.ceil($scope.service.pduportactionlogData['count'] / $scope._pageCount)) {
            $scope.service._thisPageCount = Math.ceil($scope.service.pduportactionlogData['count'] / $scope._pageCount);
        } else {
