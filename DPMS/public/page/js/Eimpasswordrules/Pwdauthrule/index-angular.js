var authruleModule = angular.module('authruleModule',
...
authruleModule.service('authruleDataService', ['$http', '$q', function ($http, $q) {
    service($q, this);
}]);

//主控制器
authruleModule.controller('authruleController', ['$scope', '$uibModal', 'i18nService', 'authruleDataService', 'uiGridGroupingConstants', 'uiGridConstants', function ($scope, $uibModal, i18nService, authruleDataService, uiGridGroupingConstants, uiGridConstants) {
    // 国际化；
    i18nService.setCurrentLang("zh-cn");
    $scope.service = authruleDataService;//要显示到页面上的数据源
    /*
        查询数据
    */
    $scope.select = function () {
        $scope.service.postData(__URL + 'Eimpasswordrules/Pwdauthrule/select_page_data', {}).then(function (data) {
    ...
    $scope.gridOptions.columnDefs = [
      ...
      { name: 'name', displayName: '规则名称', width: '10%', enableFiltering: true, cellClass: 'text-center' },
      ...
      {
          name: 'expirationtime', displayName: '到期时间', width: '10%', enableFiltering: false, cellClass: 'text-center', type: 'date',
          cellTemplate: function (grid, row, col, rowRenderIndex, colRenderIndex) {
              return '<span ng-bind="grid.appScope.convertTime(row.entity.expirationtime)"></span>'
          } 
      },
      { name: 'remark', displayName: '备注', width: '10%', enableFiltering: false, cellClass: 'text-center' },
      {
      ...
      },
      {
          ...
          cellTemplate: function (grid, row, col, rowRenderIndex, colRenderIndex) {
              return '<a href="javascript:;" class="green top-5"  ng-click="grid.appScope.edit(grid, row, col, rowRenderIndex, colRenderIndex,1)"><i class="glyphicon glyphicon-pencil bigger-120"></i></a>&nbsp;&nbsp;<a href="javascript:;" class="red top-5"  ng-click="grid.appScope.remove(grid, row, col, rowRenderIndex, colRenderIndex,0)"><i class="glyphicon glyphicon-trash bigger-120"></i></a> '
       }}
    ];
    //启用禁用状态切换
    $scope.changestatus = function (item) {
        ...
        item.status = num;
        axios.post(__URL + "Eimpasswordrules/Pwdauthrule/update_page_data", params)
             .then(function (response) {
    ...
    $scope.updateData = function () {
        ...
        $scope.authruleDataService = $scope.usergroup.slice(sIndex, eIndex);
        console.log($scope.authruleDataService);
    }
    ...
    */
    $scope.open = function () {
        $scope.service.openModal('saveusergroup.html', 'saveAuthruleController', $uibModal).then(
            function (data) {
                //close 方法的回调
            },
            function (data) {
...
//添加编辑用户组的控制器
authruleModule.controller('saveAuthruleController', ["$scope", "$uibModalInstance", 'authruleDataService', function ($scope, $uibModalInstance, authruleDataService) {
    $scope.Source = angular.copy(authruleDataService);
    $scope.url = __URL + 'Eimpasswordrules/Pwdauthrule/';
        ...
        $scope.Source.postData($scope.url, $scope.params).then(function (data) {
            switch ($scope.Source.Action) {
                case 0:
                    ...
                    //更新service数据源
                    authruleDataService.addData(data);
                    if (status == 0) {
                        //status 0一次添加 
                ...
                case 1:
                    if (data['updatedExisting']) {
                        //修改成功                        
                        authruleDataService.updateData($scope.Source.selectItem);
                ...
                case 2:
                    ...
                    if (data.lastOp.inc == 1) {
                        authruleDataService.delData(authruleDataService.selectItem);
                        alert('删除成功');
                        $uibModalInstance.close();
                        break;
                    }
...
//tree的控制器
authruleModule.controller('treeController', ['$scope', "$uibModalInstance", 'authruleDataService', function ($scope, $uibModalInstance, source) {
    //取消按钮
    $scope.cancel = function () {
        $uibModalInstance.dismiss('cancel');
    };
