...
.controller('MainController', function ($scope, dataService, $rootScope, $http, $timeout, $location/**, cfpLoadingBar*/, $q) {
    $scope.service = dataService;
    $scope.select_systemsettingData = function () {
        $http({
            method: 'post',
            url: '/index.php/Eimsystemsetting/Systemsetting/select_page_data',
            data: { $json: true },
            ...
            // 请求成功执行代码
            $scope.service.privateDateObj.systemsettingData = response.data;
            if (Object.keys($scope.service.privateDateObj.systemsettingData).length > 0) {
                angular.forEach($scope.service.privateDateObj.systemsettingData, function (value, key) {
                    angular.forEach(value, function (val, k) {
                        if (val.indexOf&&val.indexOf("{") > -1) {
                            $scope.service.privateDateObj.systemsettingData[key][k] = JSON.parse(val);
                            angular.forEach($scope.service.privateDateObj.systemsettingData[key][k], function (arr, index) {
                                if (arr.split) {
                                    $scope.service.privateDateObj.systemsettingData[key][k][index] = arr.split(',');
                                }
                            });
                        }
                    })
    ...
    //系统设置项
    $scope.select_systemsettingData();
