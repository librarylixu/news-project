...
window.Win10 = {
    ...
    _closeWin:function (index) {
        ...
        delete Win10._openPageInfoDict[Win10._openPageInfo[index]];
        delete Win10._openPageInfo[index];
        Win10._renderBar();
    },
    ...
    _init:function () {
        ...
        Win10.setContextMenu('#win10>.desktop',[
            ['<i class="fa fa-fw fa-star"></i> 收藏本页',function () {
             ...
             ['<i class="fa fa-edit"></i> ' + Win10.lang('修改密码', 'Edit Password'), function () { Win10.editpwd() }],
            ...
            ['<i class="fa  fa-power-off"></i> ' + Win10.lang('安全退出', 'Exit'), function () { window.location.href = '/index.php/Home/Login/logout'; }],
        ]);
    ...
    editpwd:function(){
        //修改密码
        this.openUrl('/index.php/Crmuser/Users/editpwdpage', '<i class="fa fa-edit icon red"></i>修改登录密码', [['350px', '220px']])
    ...
    openUrl: function (url, title, areaAndOffset) {
        if (Win10._openPageInfoDict[url] != undefined) {
            layer.msg("该页面已经打开了");
            return false;
        }
        else if (this._countTask > 12) {
        ...
        //layer.full(index);
        //防止多开
        this._openPageInfo[index] = url;
        this._openPageInfoDict[url] = index;
    ...
    aboutUs: function() {
        ...
        layer.open({
            ...
            title: 'CRM '+this._version,
            shadeClose: true, //开启遮罩关闭
            area: ['320px', '200px'], //宽高
            content: '<div style="padding: 10px;font-size: 12px">' +
            '<p>北京昕辰清虹科技有限公司</p>' +
            '<p>Copyright © 2016-2018 By xinchen Allrights Reserved.</p>' +
            '<p>010-64126508</p>' +
            '</div>'
        });
    },
    opinionFeedback: function () {
        //关于我们
        layer.open({
            type: 1,
            closeBtn: 1, //不显示关闭按钮
            ...
            anim: 2,
            skin: 'layui-layer-molv',
            title: '意见反馈 ' + this._version,
            shadeClose: true, //开启遮罩关闭
            area: ['420px', '300px'], //宽高
            content: '<div style="padding: 10px;font-size: 12px">' +
            '<p>意见反馈</p>' +
            '<input type="text" placeholder="填写您的宝贵意见">' +
            '<button>提交</button>' +
            '</div>'
        });
    },
