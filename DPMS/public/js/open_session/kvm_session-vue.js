...
window.onload = function () {
    vm = new Vue({
        ...
        methods: {
            ...
            check_device_sn: check_device_sn,
            _get_pwd: _get_pwd,
            _setEnmuPwd: _setEnmuPwd,
            _enmuPwd: _enmuPwd,
            on_select_pwd: on_select_pwd,
            _openUrl: _openUrl,
            openWebSShWindows: openWebSShWindows
        },
        created: function () {
...
function check_device_sn(p_d_oid) {
    axios.post(__APP + "/Eim/SessionTools/async_check_device_sn", { 'deviceoid': p_d_oid })
            .then(function (msg) {
                if (msg == "ok") {
                    //2.获取密码
                    _get_pwd();
...
function _get_pwd() {
    ...
    //axios.post(__URL + "/HuiKVM/get_openSession_pwd", postdata)
    //       .then(function (msg) {
    //           if (msg == "ok") {
    //               //2.获取密码
    //               _get_pwd();
...
function _enmuPwd(pwdlist,gatewaylist) {
    ...
    if (gatewaylist) {
        ...
        $.each(gatewaylist, function (i, v) {
            ...
            layerhtml += "\
            <div class='radio-box'>\
            <input id='inputgateway-{P_id}' name='gateway' type='radio'  value='{P_id}' checked={P_checked} />\
            <label for='inputgateway-{P_idd}' style='cursor:pointer;' >{P_ip}</label>\
            </div>".format({ P_id: _id, P_idd: _id, P_ip: ip, P_checked: _check });
        });      
